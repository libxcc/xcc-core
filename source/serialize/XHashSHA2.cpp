...
#include <source/serialize/XHashTypes.h>
...
static void sha_end1(unsigned char hval[], XCC_CORE_SHA256_CONTEXT ctx[1], const unsigned int hlen)
{
...
{
	memset(ctx, 0, sizeof(XCC_CORE_SHA256_CONTEXT));
...
static void sha_end2(unsigned char hval[], XCC_CORE_SHA512_CONTEXT ctx[1], const unsigned int hlen)
{
...
{
	memset(ctx, 0, sizeof(XCC_CORE_SHA384_CONTEXT));
...
{
	memset(ctx, 0, sizeof(XCC_CORE_SHA512_CONTEXT));
...
_XCOREAPI_ int __xcall__ sha2_begin(unsigned long len, XCC_CORE_SHA2_CONTEXT ctx[1])
{
	switch(len)
	{
#if defined(SHA_224)
